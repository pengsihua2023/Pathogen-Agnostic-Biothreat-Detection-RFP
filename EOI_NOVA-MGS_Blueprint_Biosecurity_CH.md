...
**申请经费（约）：** $301,400（直接费用 + 间接费用，详见第 6 节）
...
*通道 A（核酸层）。* 通过 sourmash 的 FracMinHash containment 与经过整理的病毒 sketch 数据库比对，保留完整的 containment 分布而非阈值判定结果。该通道用于捕获那些与已知病毒相关、但落在分类器归属阈值以下的病原体。

*通道 B（蛋白层）。* 对组装 contig 与高复杂度 read 簇进行六框 ORF 预测，随后用蛋白语言模型（ESM-2）生成嵌入，并针对由已知病毒蛋白组构建的嵌入流形做基于密度的离群检测。该通道的设计目标是找回那些核酸序列已经发散到无法识别、但衣壳、聚合酶或糖蛋白折叠仍保留病毒特征的病原体——这恰恰是通道 A 失效的区间。

两条通道的输出经融合后，用等渗回归（isotonic regression）对照留出的真值集（见目标 3）进行校准，产出每条序列的 **新颖性与关切度评分（Novelty and Concern Score, NCS）**。该分数是可解释的概率，而非任意量纲的指标。真正的交付价值在于**校准质量**而非原始区分能力——只有经过校准，下游的告警阈值才站得住脚。
...
| 类别 | 金额 |
|---|---|
| 项目负责人投入（40% FTE） | $62,000 |
| 博士后研究员（100% FTE） | $78,000 |
| 研究生助研（50% FTE，含学费） | $46,000 |
| 附加福利（Fringe benefits） | $41,000 |
| 计算、存储与云端弹性算力 | $35,000 |
| 论文发表、成果传播与差旅 | $12,000 |
| **直接费用合计** | **$274,000** |
| 间接费用（按 Blueprint 政策，取直接费 10%） | $27,400 |
| **申请总额** | **$301,400** |

间接费严格按 Blueprint 公布的 10% 上限编制；佐治亚大学协商 F&A 费率高于此值，我们正**在现阶段**（而非等到 Full Proposal 阶段）通过校科研办公室启动机构豁免申请。本项目在设计上是模块化的——若 Blueprint 倾向更紧凑的范围，**目标 1 与目标 3 可构成一个自洽的独立项目，约 $190,000**。

## 7. 风险与应对

**蛋白语言模型在碎片化环境 ORF 上的表现尚未验证。** 已在第 2 月安排通道 B 的 go/no-go 可行性测试；若通道 B 表现不达预期，仅凭通道 A 仍足以支撑一个范围收窄的交付版本。
