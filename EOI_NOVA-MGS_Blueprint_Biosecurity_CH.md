...
**申请经费（约）：** $385,000（直接费用 + 间接费用，详见第 6 节）
...
*说明：间接费用回收比例将在提交完整提案前，依据 Blueprint 公布的《间接费用政策》（Policy on Indirect Costs）与佐治亚大学 F&A rate 进行核对；上表数字为待核对的占位值。* 本项目在设计上是模块化的——若 Blueprint 倾向更紧凑的范围，目标 1 与目标 3 可构成一个自洽的独立项目，约 $240,000。
