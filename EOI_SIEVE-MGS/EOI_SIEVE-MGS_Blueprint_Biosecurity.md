...
**Distillation may not transfer across clades.** The central assumption, tested first: a month-3 ablation against an identically sized model trained without the representation-matching term, stratified by AAI. If the term buys nothing, the fallback is a larger student — still two orders of magnitude faster than the teacher — and the result is itself publishable.
