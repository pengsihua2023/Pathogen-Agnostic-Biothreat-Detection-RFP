...
*Companion submission disclosure:* our group is separately submitting a TA1 EOI (**SIEVE-MGS**, distilled read-level novelty screening with a guaranteed recall bound), led by Dr. Peng as Principal Investigator. The two are complementary but independent: SIEVE-MGS decides which reads are worth keeping, DPD decides when a trajectory warrants an alert and what that capability costs. Each is fully self-contained, independently scoped, independently budgeted, and separately led. Neither depends on the other being funded.
